...
def main():
    ...
    parser.add_argument(
        "--out", default="design/assets/favicons", help="Output directory"
    )
    parser.add_argument(
        "--skip-og",
        action="store_true",
        help="Skip the 1200x630 OG-card render (favicon sizes only)",
    )
    ...
    try:
        ...
        print(
            "Pillow not installed. Run `pip install Pillow` (needed for centering + writing the multi-size .ico).",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not args.skip_og:
        ...
        cairosvg.svg2png(
            url=args.svg_path, write_to=temp_png, output_width=w, output_height=h
        )
        img = Image.open(temp_png).convert("RGBA")
        scale = (
            min(w / img.width, h / img.height) * 0.6
        )  # logo mark shouldn't fill the whole OG card
        new_size = (max(1, int(img.width * scale)), max(1, int(img.height * scale)))
        img = img.resize(new_size, Image.LANCZOS)
        ...
        canvas.save(dest)
        print(
            f"Exported {dest} ({w}x{h}) — a bare centered mark; if the OG card needs a wordmark/background too, compose that in code rather than relying on this alone."
        )
