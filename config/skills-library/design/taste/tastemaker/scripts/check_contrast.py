...
def report(label, hex1, hex2, floor=AA_NORMAL):
    ...
    print(
        f"[{status}] {label}: {r:.2f}:1 (floor {floor}:1) — #{hex1.lstrip('#')} vs #{hex2.lstrip('#')}"
    )
    return r >= floor
...
def run_matrix(roles):
    ...
    print(
        f"  Text-safe   (body text, links, button labels on a fill; >=4.5): {', '.join(text_safe) or 'none'}"
    )
    print(
        f"  UI-safe     (large text, icons, and borders that convey state; >=3.0 and <4.5): {', '.join(ui_safe) or 'none'}"
    )
    print(
        f"  Decorative  (below 3.0; fine as a subtle hairline, must NOT be the only thing conveying state): {', '.join(decorative) or 'none'}"
    )
    return pairs
...
def main():
    ...
    if args[0] == "--palette":
        ...
        if missing:
            print(
                f"--palette requires at least text= and bg=; missing {missing}",
                file=sys.stderr,
            )
            sys.exit(1)

        all_pass &= report(
            "body text / background", roles["text"], roles["bg"], AA_NORMAL
        )
        ...
        if "primary" in roles:
            white_ok = report(
                "white label / primary fill", "ffffff", roles["primary"], AA_NORMAL
            )
            dark_ok = report(
                f"dark text ({roles['text']}) / primary fill",
                roles["text"],
                roles["primary"],
                AA_NORMAL,
            )
            if not (white_ok or dark_ok):
                print(
                    f"  -> NEITHER white nor {roles['text']} text is readable on primary #{roles['primary']} — darken/lighten primary, don't just pick a label color and hope."
                )
            all_pass &= white_ok or dark_ok
            all_pass &= report(
                "primary / background (visibility, UI-component floor)",
                roles["primary"],
                roles["bg"],
                AA_LARGE_OR_UI,
            )
        ...
        if "accent" in roles:
            all_pass &= report(
                "accent / background (visibility + hyperlink-text floor)",
                roles["accent"],
                roles["bg"],
                AA_LARGE_OR_UI,
            )
        ...
        if len(args) != 2:
            print(
                "Usage: check_contrast.py <hex1> <hex2>  OR  --palette role=hex ...  OR  --matrix role=hex ...",
                file=sys.stderr,
            )
            sys.exit(1)
