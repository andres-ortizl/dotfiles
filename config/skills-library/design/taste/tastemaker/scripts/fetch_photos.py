...
def fetch_openverse(query, count, out):
    ...
    if not results:
        print(
            f"No CC0/public-domain results for '{query}' even after broadening. "
            "Try different terms, or --source pixabay for a differently-curated pool.",
            file=sys.stderr,
        )
        return []
        ...
        saved.append(
            {
                ...
                "openverse_url": r.get("foreign_landing_url")
                or r.get("detail_url")
                or "",
            }
        )
...
def fetch_pixabay(query, count, out):
    ...
    if not key:
        print(
            ...
            "or drop --source pixabay to use keyless Openverse instead.",
            file=sys.stderr,
        )
        return []
    params = {
...
def main():
    ...
    if not saved:
        print(
            ...
            "grey box as if it were a real photo.",
            file=sys.stderr,
        )
        sys.exit(1)
