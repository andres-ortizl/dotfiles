...
def main():
    ...
    parser.add_argument(
        "--mood",
        choices=list(MOOD_ICON_SETS.keys()),
    ...
    parser.add_argument(
        "--color", default=None, help='Hex tint applied server-side, e.g. "#6C5CE7"'
    )
    parser.add_argument(
        "--width", type=int, default=48, help="Icon width in px (default 48)"
    )
    ...
    if args.icon_set:
        ...
        print(
            f"Mood '{args.mood}' -> '{args.icon_set}' (picked from {candidates}; pass --seed to reproduce this choice).",
            file=sys.stderr,
        )
    else:
    ...
    if args.search:
        ...
        if not names:
            print(
                f"No icons found for '{args.search}' in set '{args.icon_set}'. Try a broader term or a different --set."
            )
            return
    ...
    if not args.icons:
        print(
            "Nothing to do. Use --search to discover names, or --icons to fetch.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    for name in args.icons:
        try:
            ...
            if len(svg) < 60:
                failed.append(name)
                print(
                    f"[miss] {name} — not found in set '{args.icon_set}' (run --search to find the right name)"
                )
                continue
