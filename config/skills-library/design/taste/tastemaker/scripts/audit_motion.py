...
def add_regex_findings(
    ...
    for match in re.finditer(pattern, text, flags):
        findings.append(
            Finding(severity, path, line_number(text, match.start()), rule, message)
        )
...
def audit_file(path: Path) -> list[Finding]:
    ...
    has_motion = bool(
        re.search(
            r"\b(transition|animation|gsap\.|animate\(|@keyframes|ScrollTrigger|motion\.)\b",
            text,
        )
    )
    has_reduced_motion = (
        "prefers-reduced-motion" in text
        or "useReducedMotion" in text
        or "matchMedia" in text
    )
    has_hover_motion = bool(
        re.search(
            r":hover[^{]*\{[^}]*\b(transform|animation|transition)\b", text, re.I | re.S
        )
    )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="Stop printing after this many findings",
    )
    ...
    for finding in findings[: args.max_findings]:
        print(
            f"{finding.severity} {finding.path}:{finding.line} [{finding.rule}] {finding.message}"
        )

    if len(findings) > args.max_findings:
        print(
            f"... {len(findings) - args.max_findings} more finding(s) hidden by --max-findings."
        )
