...
def summarize_mood(colors):
    ...
    notes.append(
        "muted/desaturated"
        if avg_sat < 0.35
        else ("vivid/saturated" if avg_sat > 0.65 else "moderately saturated")
    )
    notes.append(
        "dark-leaning"
        if avg_val < 0.4
        else ("light-leaning" if avg_val > 0.75 else "mid-tone")
    )
    return notes
...
def analyze_image(path):
    ...
    return {
        ...
        "wcag_aa_normal_text_pass": contrast_ratio(lightest["rgb"], darkest["rgb"])
        >= 4.5,
