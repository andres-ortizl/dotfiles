...
def solve_link(hue, chroma, bg, prefer_l):
    ...
    for target in (4.5, 3.0):
        for _ in range(6):
            got = _best_l(
                hue, c, bg, want=lambda hx: True, floor=target, prefer_l=prefer_l
            )
            if got:
                return got[1], target
            c *= 0.9
...
def generate(mood, mode, rng):
    ...
    accent, accent_floor = solve_link(
        accent_h, primary_chroma, bg, 0.70 if dark else 0.58
    )
    ...
    return {
        ...
        "mode": mode,
        "harmony": harmony,
        "base_hue": round(base_h, 1),
        "accent_hue": round(accent_h, 1),
        "roles": {
            "text": text,
            "bg": bg,
            "surface": surface,
...
def main():
    ...
    ap.add_argument(
        ...
        choices=["light", "dark"],
        default=None,
        help="Override the mood's default light/dark.",
    )
    ap.add_argument(
        "--seed",
        type=int,
        default=None,
        help="Reproducible run; omit for a fresh palette each time.",
    )
    args = ap.parse_args()
    ...
    print(
        f"# Generated palette — {p['mood']} / {p['mode']} / {p['harmony']} harmony "
    ...
    print(
        f"  (accent is legible to a floor of {p['accent_floor']}:1 vs the background)"
    )
    print()
    run_matrix(
        {
