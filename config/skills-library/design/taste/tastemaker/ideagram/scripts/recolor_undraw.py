...
def build_map(brand, extras):
    ...
    for accent in UNDRAW_ACCENTS:
        m[accent] = (
            brand
            if shade_factors[accent] == 1.0
            else shade(brand, shade_factors[accent])
        )
    for pair in extras:
        frm, to = pair.split(":")
        m[frm.lower()] = to
...
def main():
    ...
    p.add_argument(
        ...
        nargs="*",
        default=[],
        help='Extra "from:to" hex remaps, e.g. "#8ed16f:#F59E0B"',
    )
    ...
    if not re.fullmatch(r"#[0-9a-fA-F]{6}", args.accent):
        print(
            f"--accent must be a 6-digit hex like #0FB5A8, got {args.accent}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if changed == 0:
        print(
            "No unDraw accent colors found. Is this actually an unDraw-style SVG? "
    ...
    print(
        f"[ok] {out} — swapped {changed} accent occurrence(s) to {args.accent}; "
