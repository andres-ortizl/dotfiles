...
def build_index(library):
    if not os.path.isdir(library):
        ...
        print(
            "Create it and drop unDraw SVGs in (free from undraw.co/illustrations), "
            "or pass --library /path/to/svgs.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not files:
        print(
            f"No .svg files in {library} yet. Grab a few from undraw.co/illustrations.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    with open(json_path, "w") as jf:
        json.dump(
            {"library": library, "count": len(entries), "illustrations": entries},
            jf,
            indent=2,
        )
...
def main():
    ...
    p.add_argument(
        ...
        help=f"Library folder (default: {DEFAULT_LIBRARY})",
    )
    args = p.parse_args()
