...
def main():
    ...
    parser.add_argument(
        "--accent", required=True, help='Target hex color, e.g. "#C9A227"'
    )
    parser.add_argument(
        "--preserve-dark",
        action="store_true",
        help="Leave near-black fills/strokes untouched",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    for path in files:
        ...
        if args.dry_run:
            print(
                f"[dry-run] {path} — would recolor {count} attribute(s) to {args.accent}"
            )
        else:
