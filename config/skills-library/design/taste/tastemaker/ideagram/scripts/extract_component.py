...
def main():
    ...
    p.add_argument(
        "--list", action="store_true", help="List transform groups to pick from"
    )
    p.add_argument(
        "--match",
        help="Substring of the target group's opening tag, e.g. a translate(...)",
    )
    ...
    if not args.match:
        print(
            'Give --list to see groups, or --match "translate(...)" to extract one.',
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(
        f"[ok] {dest} — extracted. Render it and read getBBox() on the group to set a tight viewBox, "
