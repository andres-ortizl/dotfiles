...
def main():
    ...
    try:
        ...
        print(
            "Pillow not installed. Run `pip install Pillow` (needed to center the illustration on each canvas size without distorting it).",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    sizes_to_export = (
        SIZES if "all" in args.sizes else {k: SIZES[k] for k in args.sizes}
    )
    ...
    for label, (w, h) in sizes_to_export.items():
        ...
        new_size = (
            max(1, int(rendered.width * scale)),
            max(1, int(rendered.height * scale)),
        )
