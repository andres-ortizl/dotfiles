...
if [ "$(sudo sbctl verify 2>/dev/null | grep -c "✗")" -gt 5 ]; then
...
if [ "$SECURE_BOOT_ENABLED" = "Disabled" ]; then
  ...
  UNSIGNED_COUNT=$(sudo sbctl verify 2>/dev/null | grep -E "(systemd-boot|BOOT.*EFI|/linux$)" | grep -c "✗")
  if [ "$UNSIGNED_COUNT" -gt 0 ]; then
    echo -e "  ${YELLOW}→${NC} Sign files: ${CYAN}sudo sbctl sign-all${NC}"
  fi

  # Check if keys are enrolled
  CUSTOM_KEYS=$(sudo sbctl list-enrolled-keys | grep -ic "sbctl\|custom")
